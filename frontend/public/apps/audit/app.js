...
const state = {
    ...
    currentUser: null // Will be populated from parent window via postMessage
};
...
// ============================================
// LISTEN TO PARENT WINDOW (Next.js) FOR USER DATA
// ============================================
window.addEventListener('message', function (event) {
    ...
    if (event.data && event.data.type === 'AUTH_DATA') {
        ...
        if (user) {
            ...
            // localStorage'a da kaydet (iframe refresh durumunda)
            ...
            // Rol bilgisini de güncelle (varsa)
            if (user.roles && user.roles.length > 0) {
                // İlk rolü al (AUDIT_ADMIN, AUDIT_INSPECTOR, vb.)
                const primaryRole = user.roles[0];
                if (primaryRole === 'AUDIT_ADMIN' || primaryRole === 'ADMIN') {
                    state.currentUserRole = 'Sistem Yöneticisi';
                } else if (primaryRole === 'AUDIT_INSPECTOR') {
                    state.currentUserRole = 'Müfettiş';
                } else if (primaryRole.includes('UNIT')) {
                    state.currentUserRole = 'Birim';
                    state.currentUserDepartment = user.department;
                }
            }

            console.log('✅ Kullanıcı state güncellendi:', state.currentUser.displayName, '- Rol:', state.currentUserRole);
        }
    }
});

// On load, check if user data is already in localStorage (from previous session)
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (savedUser && !state.currentUser) {
        try {
            ...
            console.log('✅ Kullanıcı localStorage\'dan yüklendi:', state.currentUser.displayName);
            updateUserDisplay();
        } catch (e) {
            console.error('localStorage kullanıcı verisi parse edilemedi:', e);
        }
    }
});
...
window.handleCreateAudit = function (event) {
    ...
    saveToStorage();
    saveAuditToAPI(auditData); // Async save to API
...
window.handleCreateFinding = function (event) {
    ...
    saveToStorage();
    saveFindingToAPI(findingData); // Async save to API
...
window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'NAVIGATE') {
        ...
        if (event.data.user) {
            ...
            // Map backend roles to audit module internal roles
            const backendRoles = event.data.user.roles || [];
            if (backendRoles.includes('ADMIN') || backendRoles.includes('AUDIT_ADMIN')) {
                state.currentUserRole = 'Sistem Yöneticisi';
            } else if (backendRoles.includes('AUDIT_INSPECTOR')) {
                state.currentUserRole = 'Müfettiş';
            } else if (backendRoles.includes('AUDIT_UNIT')) {
                state.currentUserRole = 'Birim';
            } else {
                state.currentUserRole = 'Görüntüleyici';
            }

            state.currentUserDepartment = event.data.user.department;
            updateUserDisplay();
        }

        // Auto-load data from API when navigating or starting
        ...
        <div class="card" style="max-width: 600px; margin: 0 auto;">
            ...
            <form style="margin-top: 1.5rem;">
                <div style="display: flex; align-items: center; gap: 1.5rem; margin-bottom: 2rem;">
                    <div style="width: 80px; height: 80px; background: var(--primary); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 2rem; font-weight: bold;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" value="${user.displayName || user.username || ''}" readonly style="background: #f3f4f6;">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label class="form-label">Birim / Departman</label>
                    <input type="text" class="form-input" value="${user.department || 'Teftiş Kurulu'}" disabled style="background: #f3f4f6;">
                </div>
                
                <div style="margin-top: 1rem; padding: 1rem; background: #eff6ff; border-radius: 0.5rem; border: 1px solid #dbeafe; font-size: 0.85rem; color: #1e40af;">
                    <i data-lucide="info" style="width: 14px; vertical-align: middle; margin-right: 0.25rem;"></i>
                    Profil bilgileri ana sistem (Kullanıcı Yönetimi) üzerinden güncellenebilir.
                </div>

                <div class="form-actions" style="margin-top: 2rem;">
                    <button type="button" class="btn btn-primary" onclick="showToast('Profil ayarları bu ekran üzerinden değiştirilemez.', 'info')">Kapat</button>
                </div>
            </form>
        </div>
    `;
...
let API_TOKEN = localStorage.getItem('access_token');

// Generic API fetch helper
async function apiRequest(endpoint, method = 'GET', data = null) {
    try {
        const token = API_TOKEN || localStorage.getItem('access_token');
        const options = {
            method,
            headers: {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                ...
                // Could notify parent to redirect to login
            }
...
async function loadDataFromAPI() {
    try {
        const [audits, findings] = await Promise.all([
            ...
            apiRequest('/audit/findings')
        ]);

        if (audits) state.audits = audits;
        if (findings) state.findings = findings;

        console.log('✅ Audit data synced from API');
        return true;
    } catch (error) {
...
// Global hook for saving specific items
async function saveAuditToAPI(audit) {
    const method = audit.id && String(audit.id).length < 15 ? 'PUT' : 'POST';
    const endpoint = method === 'PUT' ? `/audit/audits/${audit.id}` : '/audit/audits';
    return apiRequest(endpoint, method, audit);
}

async function saveFindingToAPI(finding) {
    return apiRequest('/audit/findings', 'POST', finding);
}

// Note: We keep saveToStorage for local backup, but primary data is now via API
async function saveDataToAPI() {
    saveToStorage(); // Local backup
    // Full sync is complex for vanilla JS state, we usually save on actions
}
...
// ============================================
// TEFTİŞ KURULU PERSONELİ EKRANI
// ============================================
// Bu kodu app.js'in EN SONUNA ekleyin (satır 11623'ten sonra)

// Initialize staff data in state
if (!state.staff) {
    state.staff = [];
}

// Staff Screen Render Function
    ...
    mainView.innerHTML = `
        <div class="page-container">
            <div class="page-header">
                <div>
                    <h1 style="margin:0; font-size:1.75rem; font-weight:700;">Teftiş Kurulu Personeli</h1>
                    <p style="margin-top:0.5rem; color:#6b7280;">Denetim ekibi üyelerinin bilgilerini görüntüleyin ve yönetin</p>
                </div>
                <button onclick="openStaffModal()" class="btn-primary" style="display:flex; align-items:center; gap:0.5rem;">
                    <i data-lucide="user-plus" style="width:18px; height:18px;"></i>
                    Yeni Personel Ekle
                </button>
            </div>
            ...
            <!-- Filters -->
            <div style="background:#fff; padding:1rem; border-radius:0.5rem; margin-bottom:1.5rem; display:flex; gap:1rem; flex-wrap:wrap;">
                <input type="text" id="staff-search" placeholder="Personel ara..."
                    oninput="filterStaff()"
                    style="flex:1; min-width:200px; padding:0.5rem; border:1px solid #e5e7eb; border-radius:0.375rem;">
                
                <select id="staff-status-filter" onchange="filterStaff()"
                    style="padding:0.5rem; border:1px solid #e5e7eb; border-radius:0.375rem; min-width:150px;">
                    <option value="">Tüm Durumlar</option>
                    <option value="active">Aktif</option>
                    <option value="inactive">Pasif</option>
                </select>

                <select id="staff-department-filter" onchange="filterStaff()"
                    style="padding:0.5rem; border:1px solid #e5e7eb; border-radius:0.375rem; min-width:150px;">
                        ...
                        ${staff.length === 0 ? `
                            <tr>
                                <td colspan="8" style="text-align:center; padding:3rem; color:#9ca3af;">
                                    ...
                                    <button onclick="openStaffModal()" class="btn-primary" style="margin-top:1rem;">
                                        İlk Personeli Ekle
                                    </button>
                                </td>
                            </tr>
...
function renderStaffRow(staff) {
    const experience = calculateExperience(staff.startDate);
    const remainingLeave = (staff.annualLeave || 0) - (staff.usedLeave || 0);

    return `
        <tr>
            <td>
                <div style="display:flex; align-items:center; gap:0.75rem;">
                    <div style="width:40px; height:40px; border-radius:50%; background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); display:flex; align-items:center; justify-content:center; color:#fff; font-weight:600; font-size:0.9rem;">
                        ${(staff.firstName || '-')[0].toUpperCase()}${(staff.lastName || '-')[0].toUpperCase()}
                    </div>
                    <div>
                        <div style="font-weight:600;">${staff.firstName} ${staff.lastName}</div>
                        <div style="font-size:0.85rem; color:#6b7280;">${staff.email}</div>
                    </div>
                </div>
            </td>
            <td>${staff.title}</td>
            <td>${staff.department}</td>
            <td>${experience} yıl</td>
            <td>
                <div style="display:flex; gap:0.25rem; flex-wrap:wrap;">
                    ${(staff.certifications || []).map(cert => `
                        <span class="badge" style="background:${getCertColor(cert)}; color:#fff; font-size:0.75rem;">
                            ${cert}
                        </span>
                    `).join('')}
                    ${(staff.certifications || []).length === 0 ? '<span style="color:#9ca3af;">-</span>' : ''}
                </div>
            </td>
            <td>
                <div style="font-size:0.85rem;">
                    <span style="color:#10b981; font-weight:600;">${remainingLeave}</span> / ${staff.annualLeave || 0} gün
                </div>
            </td>
            <td>
                <span class="badge" style="background:${staff.isActive ? '#10b981' : '#ef4444'}; color:#fff;">
                    ${staff.isActive ? 'Aktif' : 'Pasif'}
                </span>
            </td>
            <td>
                <div style="display:flex; gap:0.5rem;">
                    <button onclick="viewStaffDetail('${staff.id}')" class="btn-icon" title="Detaylar">
                        <i data-lucide="eye" style="width:16px; height:16px;"></i>
                    </button>
                    <button onclick="editStaff('${staff.id}')" class="btn-icon" title="Düzenle">
                        <i data-lucide="edit" style="width:16px; height:16px;"></i>
                    </button>
                    <button onclick="deleteStaff('${staff.id}')" class="btn-icon-danger" title="Sil">
...
window.filterStaff = function () {
    ...
    if (tbody) {
        ...
        ` : filtered.map(s => renderStaffRow(s)).join('');
        lucide.createIcons();
    }
};
...
window.openStaffModal = function (staffId = null) {
    ...
    const staff = isEdit ? state.staff.find(s => s.id === staffId) : null;

    const modalHtml = `
        <div id="staff-modal" class="modal-overlay">
            <div class="modal" style="max-width:700px;">
                <div class="modal-header">
                    <h2 class="modal-title">${isEdit ? 'Personel Düzenle' : 'Yeni Personel Ekle'}</h2>
                    <button class="close-modal" onclick="closeStaffModal()">
                        <i data-lucide="x" style="width:24px;"></i>
                    </button>
                </div>
                <form onsubmit="handleStaffSubmit(event)" id="staff-form">
                    <input type="hidden" name="id" value="${isEdit ? staff.id : ''}">
                    
                    <div class="form-row">
                        <div class="form-group">
                            <label>Ad *</label>
                            <input type="text" name="firstName" required value="${isEdit ? staff.firstName : ''}">
                        </div>
                        <div class="form-group">
                            <label>Soyad *</label>
                            <input type="text" name="lastName" required value="${isEdit ? staff.lastName : ''}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="title" required>
                            <option value="">Seçiniz</option>
                            <option ${isEdit && staff.title === 'Başkan' ? 'selected' : ''}>Başkan</option>
                            <option ${isEdit && staff.title === 'Başkan Yardımcısı' ? 'selected' : ''}>Başkan Yardımcısı</option>
                            <option ${isEdit && staff.title === 'Baş Müfettiş' ? 'selected' : ''}>Baş Müfettiş</option>
                            <option ${isEdit && staff.title === 'Müfettiş' ? 'selected' : ''}>Müfettiş</option>
                            <option ${isEdit && staff.title === 'Müfettiş Yardımcısı' ? 'selected' : ''}>Müfettiş Yardımcısı</option>
                        </select>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label>Email *</label>
                            <input type="email" name="email" required value="${isEdit ? staff.email : ''}">
                        </div>
                        <div class="form-group">
                            <label>Telefon</label>
                            <input type="tel" name="phone" value="${isEdit ? (staff.phone || '') : ''}">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="date" name="birthDate" value="${isEdit ? (staff.birthDate || '') : ''}">
                        </div>
                        <div class="form-group">
                            <label>Departman *</label>
                            <select name="department" required>
                                <option value="">Seçiniz</option>
                                <option ${isEdit && staff.department === 'İç Denetim' ? 'selected' : ''}>İç Denetim</option>
                                <option ${isEdit && staff.department === 'Risk Yönetimi' ? 'selected' : ''}>Risk Yönetimi</option>
                                <option ${isEdit && staff.department === 'Uyum' ? 'selected' : ''}>Uyum</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="startDate" required value="${isEdit ? staff.startDate : ''}">
                    </div>

                    <div class="form-group">
                        <label>Sertifikalar</label>
                        <div style="display:flex; flex-wrap:wrap; gap:0.75rem; padding:0.75rem; background:#f9fafb; border-radius:0.375rem;">
                            ${['CIA', 'CISA', 'CFE', 'CRMA', 'Diğer'].map(cert => `
                                <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
                                    <input type="checkbox" name="certifications" value="${cert}" 
                                        ${isEdit && (staff.certifications || []).includes(cert) ? 'checked' : ''}>
                                    <span>${cert}</span>
                                </label>
                            `).join('')}
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:0.75rem; padding:0.75rem; background:#f9fafb; border-radius:0.375rem;">
                            ${['IT Denetimi', 'Finansal Denetim', 'Operasyonel Denetim', 'Uyum Denetimi', 'Risk Yönetimi'].map(area => `
                                <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
                                    <input type="checkbox" name="specializations" value="${area}"
                                        ${isEdit && (staff.specializations || []).includes(area) ? 'checked' : ''}>
                                    <span style="font-size:0.9rem;">${area}</span>
                                </label>
                            `).join('')}
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="annualLeave" required min="0" value="${isEdit ? staff.annualLeave : 20}">
                        </div>
                        <div class="form-group">
                            <label>Kullanılan İzin (gün)</label>
                            <input type="number" name="usedLeave" min="0" value="${isEdit ? (staff.usedLeave || 0) : 0}">
                        </div>
                    </div>

                    <div class="form-group">
                        <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
                            <input type="checkbox" name="isActive" ${isEdit ? (staff.isActive ? 'checked' : '') : 'checked'}>
                            <span>Aktif</span>
                        </label>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn-secondary" onclick="closeStaffModal()">İptal</button>
...
window.handleStaffSubmit = function (event) {
    event.preventDefault();
    const form = event.target;
    const formData = new FormData(form);

    const staffId = formData.get('id');
    const isEdit = staffId && staffId.trim() !== '';
    ...
    const staffData = {
        id: isEdit ? staffId : Date.now().toString(),
        ...
        certifications,
        specializations,
        annualLeave: parseInt(formData.get('annualLeave'), 10),
        usedLeave: parseInt(formData.get('usedLeave') || '0', 10),
        isActive: formData.get('isActive') === 'on',
        createdAt: isEdit ? state.staff.find(s => s.id === staffId)?.createdAt : new Date().toISOString()
    };
...
window.viewStaffDetail = function (staffId) {
    ...
    const experience = calculateExperience(staff.startDate);
    const remainingLeave = (staff.annualLeave || 0) - (staff.usedLeave || 0);
    const age = staff.birthDate ? Math.floor((new Date() - new Date(staff.birthDate)) / (1000 * 60 * 60 * 24 * 365)) : '-';

    const modalHtml = `
        <div id="staff-detail-modal" class="modal-overlay">
            <div class="modal" style="max-width:600px;">
                <div class="modal-header">
                    <h2 class="modal-title">Personel Detayları</h2>
                    <button class="close-modal" onclick="closeStaffDetailModal()">
                        <i data-lucide="x" style="width:24px;"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div style="text-align:center; margin-bottom:1.5rem;">
                        <div style="width:80px; height:80px; border-radius:50%; background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); display:inline-flex; align-items:center; justify-content:center; color:#fff; font-weight:700; font-size:1.5rem; margin-bottom:1rem;">
                            ${(staff.firstName || '-')[0].toUpperCase()}${(staff.lastName || '-')[0].toUpperCase()}
                        </div>
                        <h3 style="margin:0;">${staff.firstName} ${staff.lastName}</h3>
                        <p style="color:#6b7280; margin:0.25rem 0;">${staff.title}</p>
                        <span class="badge" style="background:${staff.isActive ? '#10b981' : '#ef4444'}; color:#fff; margin-top:0.5rem;">
                            ${staff.isActive ? 'Aktif' : 'Pasif'}
                        </span>
                    </div>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                        <div>
                            ...
                            <p style="margin:0.25rem 0 0; font-weight:600;">${staff.email}</p>
                        </div>
                        <div>
                            <p style="font-size:0.85rem; color:#6b7280; margin:0;">Telefon</p>
                            <p style="margin:0.25rem 0 0; font-weight:600;">${staff.phone || '-'}</p>
                        </div>
                        <div>
                            <p style="font-size:0.85rem; color:#6b7280; margin:0;">Departman</p>
                            <p style="margin:0.25rem 0 0; font-weight:600;">${staff.department}</p>
                        </div>
                        <div>
                            <p style="font-size:0.85rem; color:#6b7280; margin:0;">Yaş</p>
                            <p style="margin:0.25rem 0 0; font-weight:600;">${age}</p>
                        </div>
                        <div>
                            <p style="font-size:0.85rem; color:#6b7280; margin:0;">Tecrübe</p>
                            <p style="margin:0.25rem 0 0; font-weight:600;">${experience} yıl</p>
                        </div>
                        <div>
                            <p style="font-size:0.85rem; color:#6b7280; margin:0;">İşe Başlama</p>
                            <p style="margin:0.25rem 0 0; font-weight:600;">${formatDate(staff.startDate)}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        <p style="font-size:0.85rem; color:#6b7280; margin:0 0 0.5rem;">Sertifikalar</p>
                        <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                            ${(staff.certifications || []).map(cert => `
                                <span class="badge" style="background:${getCertColor(cert)}; color:#fff;">
                                    ${cert}
                                </span>
                            `).join('')}
                            ${(staff.certifications || []).length === 0 ? '<span style="color:#9ca3af;">Yok</span>' : ''}
                        </div>
                    </div>

                    <div style="margin-top:1rem;">
                        <p style="font-size:0.85rem; color:#6b7280; margin:0 0 0.5rem;">Uzmanlık Alanları</p>
                        <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                            ${(staff.specializations || []).map(area => `
                                <span class="badge" style="background:#6b7280; color:#fff;">
                                    ${area}
                                </span>
                            `).join('')}
                            ${(staff.specializations || []).length === 0 ? '<span style="color:#9ca3af;">Yok</span>' : ''}
                        </div>
                    </div>
                    ...
                    <div style="background:#f9fafb; padding:1rem; border-radius:0.5rem;">
                        <p style="font-size:0.85rem; color:#6b7280; margin:0 0 0.5rem;">İzin Durumu</p>
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div>
                                <p style="margin:0; font-size:0.9rem;">Toplam: <strong>${staff.annualLeave || 0} gün</strong></p>
                                <p style="margin:0.25rem 0 0; font-size:0.9rem;">Kullanılan: <strong style="color:#ef4444;">${staff.usedLeave || 0} gün</strong></p>
                            </div>
                            <div style="text-align:right;">
                                <p style="margin:0; font-size:1.25rem; font-weight:700; color:#10b981;">${remainingLeave}</p>
                                <p style="margin:0; font-size:0.85rem; color:#6b7280;">Kalan</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button class="btn-secondary" onclick="closeStaffDetailModal()">Kapat</button>
                    <button class="btn-primary" onclick="closeStaffDetailModal(); editStaff('${staff.id}')">
                        Düzenle
                    </button>
                </div>
            </div>
...
console.log("✅ Teftiş Kurulu Personeli modülü yüklendi");
// ============================================
// TEFTİŞ KURULU PERSONELİ EKRANI
// ============================================
// Bu kodu app.js'in EN SONUNA ekleyin (satır 11623'ten sonra)
